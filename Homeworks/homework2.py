...
class Interval(object):  
    ...
    def __pow__(self, other):
        ...
        if other%2==0 and other>0:#even
            if self.leftendpoint>=0:
                ...
                powe.append(0)
                powe.append(max(self.leftendpoint**other,self.rightendpoint**other))
        else:
            powe.append(self.leftendpoint**other)
            powe.append(self.rightendpoint**other)
    ...
    def __contains__(self):
        if not isinstance(self.leftendpoint,int) or isinstance(self.leftendpoint,float):
            raise TypeError('Left endpoint is not real value')
        if not isinstance(self.rightendpoint,int) or isinstance(self.rightendpoint,float) :
